...
import static net.runelite.client.RuneLite.WORLD_1_IP;
import static net.runelite.http.api.worlds.WorldType.MEMBERS;
...
public class Worlds
{
	private static List<World> lookup()
	{
		...
		WorldResult lookup = constructWorlds();
		if (lookup == null)
		{
	...
	private static WorldResult constructWorlds() {
		WorldResult worldResults = new WorldResult();
		List<net.runelite.http.api.worlds.World> worlds = new ArrayList<>();
		net.runelite.http.api.worlds.World world1 = new net.runelite.http.api.worlds.World(1, EnumSet.of(MEMBERS), WORLD_1_IP, ContentConstants.SERVER_NAME + " Main", 1, 2000);
		worlds.add(world1);
		worldResults.setWorlds(worlds);
		return worldResults;
	}
	...
	public static List<World> getAll(Predicate<World> filter)
	{
		...
		Static.getClient().setWorldList(loadedWorlds.toArray(World[]::new));
